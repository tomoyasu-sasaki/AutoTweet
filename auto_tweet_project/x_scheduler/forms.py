...
class TweetScheduleForm(forms.ModelForm):
    ...
    # デフォルトで現在から設定された時間後を設定
    scheduled_time = forms.DateTimeField(
        label="予定時刻",
        widget=forms.DateTimeInput(
            attrs={"type": "datetime-local"}, format="%Y-%m-%dT%H:%M"
        ),
        initial=timezone.now() + timedelta(hours=int(os.getenv("DEFAULT_SCHEDULE_HOURS", "1"))),
    )
        ...
        widgets = {
            "content": forms.Textarea(
                attrs={
                    ...
                    "class": "form-control",
                    "maxlength": int(os.getenv("MAX_TWEET_LENGTH", "280")),
                }
            ),
        ...
        if image:
            ...
            if ext not in allowed_extensions:
                raise forms.ValidationError(
                    "対応している画像形式は、jpg、jpeg、png、gifのみです。"
                )
