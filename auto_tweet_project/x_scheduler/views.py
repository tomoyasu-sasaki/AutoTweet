...
from .utils import post_tweet, get_tweepy_api, get_tweepy_client
...
def schedule_list(request):
    ...
    return render(request, 'x_scheduler/schedule_list.html', {
        'schedules': schedules,
    })
...
def schedule_create(request):
        ...
        if form.is_valid():
            ...
            if 'image' in request.FILES:
                ...
                if not os.path.exists(tweet_images_dir):
                    logger.debug(f"ディレクトリが存在しないため作成します: {tweet_images_dir}")
    ...
    return render(request, 'x_scheduler/schedule_form.html', {
        'form': form,
        'title': '新規投稿スケジュール作成',
    })
...
def schedule_delete(request, pk):
    ...
    return render(request, 'x_scheduler/schedule_confirm_delete.html', {
        'tweet': tweet,
    })
